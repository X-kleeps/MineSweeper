...
def classic_Minesweeper():
    ...
    class MyButton (tk.Button):
        ...
        def __init__(self, master, x, y, number=0, *args, **kwargs): #Переопределение кнопок
                ...
                self.count_bomb = 0
                self.is_open = False
        def __repr__(self): #ОбЪект внутри консоли
            return f'MyButton {self.x} {self.y} {self.number} {self.is_mine}' #Проверка атрибутов (Ряд/Колонка)
    ...
    class MineSweeper:
        ...
        def __init__(self): #Авто запуск (Инцилизация игры)
            self.buttons = []
        ...
        def click(self, cliked_button:MyButton): #Обработка нажатия на кнопку
        ...
        def breadth_first_search (self, btn:MyButton): #Поиск в ширину для нуля
            queue = [btn]
            while queue:
        ...
        def create_button(self):
            ...
            count = 1
            for i in range(1,MineSweeper.ROW+1): #Присваивание кнопок к полю
                for j in range(1,MineSweeper.COLUMS+1):
                    btn=self.buttons[i][j]
                    btn.number = count
        ...
        def open_all_butttons(self): #Барьерные элементы
        ...
        def name_win(self): #Окно для ника
            global name,Nickmane

            def write_rezult():
                if not Nickmane.get().strip(): # проверка на пустое значение
                    showerror('Ошибка', 'Введено неправильное значение')
                    return                
        ...
        def create_consol_button(self): #Игровое поле в консоли
            for i in range(1,MineSweeper.ROW+1):
                for j in range(1,MineSweeper.COLUMS+1):
                    btn=self.buttons[i][j]
                    if btn.is_mine:
        ...
        def insert_mines(self, number: int): #Распологаем мины
        ...
        def count_mines_in_buttons(self): #Поиск мин вокруг кнопки
            for i in range(1,MineSweeper.ROW+1):
                for j in range(1,MineSweeper.COLUMS+1):
                    btn=self.buttons[i][j]
                    count_bomb = 0
        ...
        @staticmethod
        def get_mines_places(exclude_number:int): #Формируем мины по индексам
            indexes=list(range(1, MineSweeper.COLUMS * MineSweeper.ROW + 1)) #Формируем числа 
